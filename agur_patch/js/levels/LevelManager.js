import { CONFIG } from '../config.js?v=202608192036';
import { Brick, biomeColor } from '../entities/Brick.js?v=202608192036';
import { LEVELS } from './Layouts88.js?v=202608192036';
...
/*
 * 88 авторских раскладок живут в Layouts88.js.
 *
 * Уровни раскопок (1-10 каждого биома):
 *   стальной внешний каркас + тематическая постройка +
 *   камера с артефактом за разрушаемыми стенами.
 *
 * Уровни боссов (11, 22, ..., 88):
 *   без стали, без артефакта — лёгкая арена для боя.
 */
export class LevelManager {
  constructor() {
    ...
    this.aliveCount = 0;
    this.totalCount = 0;
    this.layoutV2 = true;      // флаг для Architectures: не перекраивать уровень
    this.artifactCell = null;  // {r, c, x, y} или null на боссе
    this.levelTitle = '';
  }
  ...
  loadLevel(n) {
    ...
    for (let r = 0; r < ROWS; r++) for (let x = 0; x < COLS; x++) {
      ...
      if (ch === 'A') {
        this.artifactCell = {
          r, c: x,
          x: offX + x * step + CONFIG.BRICK.WIDTH / 2,
          y: offY + r * (CONFIG.BRICK.HEIGHT + 4) + CONFIG.BRICK.HEIGHT / 2,
        };
        continue;
      }
      const b = new Brick(offX + x * step, offY + r * (CONFIG.BRICK.HEIGHT + 4), null, r, x);
    ...
    /*
     * Кирпичи, охраняющие камеру артефакта, помечаем isVault:
     * биомные эффекты (напр. случайная сталь Загроса) не должны
     * запечатывать камеру насмерть.
     */
    if (this.artifactCell) {
      const { r: ar, c: ac } = this.artifactCell;
      for (const b of bricks) {
        if (Math.max(Math.abs(b.row - ar), Math.abs(b.col - ac)) <= 3) b.isVault = true;
      }
    }
