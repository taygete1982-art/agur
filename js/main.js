...
class Game {
  ...
  destroyBrick(brick) {
    if (this.levelManager.aliveCount <= 1 && this.state === GAME_STATE.PLAYING) this.triggerDramaticSlowMo();
  ...
  draw() {
    ...
    // ===== Z-осколки: перспективный полёт в камеру =====
    if (this.zShards.length) {
      ctx.save();
      ctx.globalCompositeOperation = 'lighter';
